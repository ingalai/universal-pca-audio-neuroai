...
def train_universal_pca_oja(training_dir, config=None, seed=None, progress_callback=None):
    ...
    # Oja uses patch sampling when max_patches is set.
    # If seed is not passed explicitly, read it from config so rng is always valid.
    seed_value = seed if seed is not None else config.get("oja_random_seed", config.get("random_seed", 0))
    rng = np.random.default_rng(seed_value)
    # Oja reads its own maximum first; fall back to the generic key for compatibility.
    max_patches = config.get("oja_max_patches_per_channel", config.get("max_patches_per_channel", 50000))
...
def train_universal_pca(training_dir, config=None, seed=None, progress_callback=None):
    """Train a universal PCA basis.

    Training policy used in this project:
        - pca_training_method = "eigh":
            Always use ALL patches from all training images.
            This gives the best closed-form PCA basis and does not use seed/rng.

        - pca_training_method = "oja":
            Use Oja/Sanger learning and obey config["oja_max_patches_per_channel"].
            This keeps Oja training controllable because iterative learning on
            all patches can be extremely slow.
    """
    config = clone_config(config)
    training_method = str(config.get("pca_training_method", "eigh")).lower()

    if training_method in ("oja", "hebbian", "hebbian_oja", "sanger", "gha"):
        return train_universal_pca_oja(
            training_dir,
            config,
            seed=seed,
    ...
    # Closed-form PCA / eigen-decomposition mode.
    # In this project, EIGH intentionally ignores max_patches_per_channel and
    # uses every patch from every training image.
    ...
    total_steps = len(paths) * 2 + len(config["channel_names"])

    # Pass 1: compute mean patch for each channel using all patches.
    for image_index, path in enumerate(paths, start=1):
        report(f"EIGH mean pass {image_index}/{len(paths)}: {path.name}", image_index, total_steps)
        rgb = crop_to_multiple(np.asarray(read_rgb_image(path), dtype=np.uint8), config["patch_size"])
        ycc = pil_rgb_to_ycbcr_array(Image.fromarray(rgb, mode="RGB"))
    ...
    # Pass 2: compute covariance using all patches.
    for image_index, path in enumerate(paths, start=1):
        report(
            f"EIGH covariance pass {image_index}/{len(paths)}: {path.name}",
    ...
    for channel_index, name in enumerate(config["channel_names"], start=1):
        report(
            f"EIGH diagonalizing covariance for channel {name}",
            len(paths) * 2 + channel_index,
            total_steps,
        )
        mean_patch, eigvecs, eigvals, explained = pca_from_mean_and_cov(
            means[name],
            covs[name],
            counts[name],
        )
        ...
        summary[name] = {
            "patches": [int(counts[name]), int(patch_dim)],
            "cum_explained_to_last_payload_pc": float(np.cumsum(explained)[last_payload_pc]),
            "training_mode": "eigh_all_patches_streaming",
            "sampling": "disabled_for_eigh",
        }

    report("EIGH PCA basis fitted with all patches", total_steps, total_steps)
    return basis, summary
